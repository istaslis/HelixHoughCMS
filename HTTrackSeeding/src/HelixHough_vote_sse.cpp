...
{
  const unsigned int zero_int_a[4] __attribute__((aligned(16))) = {0x00000000,0x00000000,0x00000000,0x00000000};
  __m128i zero_int = _mm_load_si128((__m128i*)zero_int_a);
  const unsigned int neg1_int_a[4] __attribute__((aligned(16))) = {0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF};
  __m128i neg1_int = _mm_load_si128((__m128i*)neg1_int_a);
  const unsigned int one_int_a[4] __attribute__((aligned(16))) = {0x00000001,0x00000001,0x00000001,0x00000001};
  ...
  __m128i low_phi_bin_1 = _mm_cvttps_epi32( (((min_phi-low_phi)*inv_phi_range)*((n_phi))) );
  __m128i high_phi_bin_1 = _mm_cvttps_epi32( (((max_phi-low_phi)*inv_phi_range)*((n_phi))) );
  __m128i low_phi_bin_2 = _mm_cvttps_epi32( (((min_phi + twopi-low_phi)*inv_phi_range)*((n_phi))) );
...
high_phi, float inv_phi_range, fastvec& vote_array)
{
  ...
  unsigned int philow1[4] __attribute__((aligned(16))) = {0x00000000,0x00000000,0x00000000,0x00000000};
  unsigned int philow2[4] __attribute__((aligned(16))) = {0x00000000,0x00000000,0x00000000,0x00000000};
  unsigned int phihi1[4] __attribute__((aligned(16))) = {0x00000000,0x00000000,0x00000000,0x00000000};
  unsigned int phihi2[4] __attribute__((aligned(16))) = {0x00000000,0x00000000,0x00000000,0x00000000};
  ...
  while(true)
  {
    ...
    fillBins4_sse(minphi_a, maxphi_a, (float)n_phi, inv_phi_range, low_phi, high_phi, philow1,philow2,phihi1,phihi2);
    
    for(unsigned int i=0;i<cur;++i)
    {
      unsigned int index = four_hits[i+offset].index;
  ...
  for(unsigned int i=0;i<hits_vec[zoomlevel]->size();i++)
  {
    ...
    if(hit_counter==4)
    {
      ...
      for(unsigned int zz=0;zz<n_z0;++zz)
      {
        ...
        dzdlRange_sse(x_a, y_a, z_a, min_cos, min_sin, max_cos, max_sin, min_kappa, max_kappa, zoomranges[zoomlevel].min_d, zoomranges[zoomlevel].max_d, min_z0_a, max_z0_a, min_dzdl_a, max_dzdl_a);
...
void HelixHough::vote(unsigned int zoomlevel)
{
  ...
  {
    unsigned int B[1<<14];
    counting_sort(vote_array.size, total_bins, vote_array.arr, B, C, bins_start, bins_end);
...
//     if(bins_vec[zoomlevel]->size() > total_bins){counting_sort(total_bins, *(bins_vec[zoomlevel]), C, bins_start, bins_end);}
