...
HelixHough::HelixHough(unsigned int n_phi, unsigned int n_d, unsigned int n_k, unsigned int n_dzdl, unsigned int n_z0, HelixResolution& min_resolution, HelixResolution& max_resolution, HelixRange& range) : remove_hits(false), vote_time(0.), xy_vote_time(0.), z_vote_time(0.), print_timings(false),separate_by_helicity(true),only_one_helicity(false),  helicity(false), check_layers(false), req_layers(0), bin_scale(1.), z_bin_scale(1.),  start_zoom(0), max_hits_pairs(0), cluster_start_bin(2)
{
