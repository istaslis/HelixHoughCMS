...
static inline void setRange(const BinEntryPair5D& bp, HelixRange& range1, HelixRange& range2, unsigned int n_phi, unsigned int n_d, unsigned int n_k, unsigned int n_dzdl, unsigned int n_z0)
{
  float dzdl_size = (range1.max_dzdl - range1.min_dzdl)/((float)n_dzdl);
  float z0_size = (range1.max_z0 - range1.min_z0)/((float)n_z0);
  float phi_size = (range1.max_phi - range1.min_phi)/((float)n_phi);
...
void HelixHough::findHelicesByPairsBegin(unsigned int min_hits, unsigned int max_hits, vector<SimpleTrack3D>& tracks, unsigned int maxtracks, unsigned int zoomlevel)
{
  ...
  for(unsigned int i=0;i<(hits_vec[zoomlevel]->size());++i)
  {
    ...
    {
      if( ( (*(hits_vec[zoomlevel]))[j].layer <= (*(hits_vec[zoomlevel]))[i].layer ) ){continue;}
...
void HelixHough::findHelicesByPairs(unsigned int min_hits, unsigned int max_hits, vector<SimpleTrack3D>& tracks, unsigned int maxtracks, unsigned int zoomlevel)
{
...
//  unsigned int tracks_at_start = tracks.size();
  ...
  makeClusters(zoomlevel, pairs_vec[zoomlevel]->size(), n_phi, n_d, n_k, n_dzdl, n_z0, min_hits, *(clusters_vec[zoomlevel]), use_clusters, is_super_bin);
  if(print_timings)
  {
    gettimeofday(&t2, NULL);
    time1 = ((double)(t1.tv_sec) + (double)(t1.tv_usec)/1000000.);
  ...
  for(unsigned int i=0, size=num_clusters[zoomlevel];i<size;++i)
  {
    ...
    for(index_iter=(*(clusters_vec[zoomlevel]))[i].hit_indexes.begin();index_iter!=(*(clusters_vec[zoomlevel]))[i].hit_indexes.end();++index_iter)
    {
      if(remove_hits == true)
      {
        if( ((*hit_used)[(*(hits_vec[zoomlevel]))[(*(pairs_vec[zoomlevel]))[*index_iter].first].index] == false) && ((*hit_used)[(*(hits_vec[zoomlevel]))[(*(pairs_vec[zoomlevel]))[*index_iter].second].index] == false) )
        {
    ...
    else if( ((zoomlevel+1)==max_zoom) || (hits_vec[zoomlevel+1]->size() <= max_hits) )
    {
      //           findTracksByPairs(*(hits_vec[zoomlevel+1]), *(pairs_vec[zoomlevel+1]), tracks, zoomranges[zoomlevel+1]);
