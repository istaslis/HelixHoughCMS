...
class HelixKalman
{
  public:
    ...
    void addHit(PHENIXHough::SimpleHit3D& hit, HelixKalmanState& state);
    
  protected:
    virtual void calculateProjections(PHENIXHough::SimpleHit3D& hit, HelixKalmanState& state, Eigen::Matrix<float,2,5>& H, Eigen::Matrix<float,2,1>& ha) = 0;
    virtual void calculateMeasurements(PHENIXHough::SimpleHit3D& hit, Eigen::Matrix<float,2,1>& m, Eigen::Matrix<float,2,2>& G) = 0;
