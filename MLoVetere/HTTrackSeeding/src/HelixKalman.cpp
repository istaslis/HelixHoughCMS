#include "MLoVetere/HTTrackSeeding/interface/HelixKalman.h"
...
void HelixKalman::subtractProjections(Matrix<float,2,1>& m, Matrix<float,2,1>& ha, Matrix<float,2,1>& diff)
{
  diff = m - ha;
}
...
// dA/dA' , where A are the global helix parameters with pivot (0,0,0), and A' are the 3 helix parameters phi,k,dzdl with the pivot being the intersection at the current layer
void HelixKalman::calculate_dAdAp(HelixKalmanState& state, Matrix<float,5,3>& dAdAp, float& phi_p, float& cosphi_p, float& sinphi_p)
{
  float phi = state.phi;
  float d = state.d;
  float k = state.kappa;
...
//   float signk = (float)(sign(k));
  ...
  if(k != 0.)
  {
    ...
    dtxdA_p = x0;
    dtydA_p = y0;
    dAdAp(1,1) = dphidtx*dtxdA_p + dphidty*dtydA_p - (sqrt( tx*tx + ty*ty ) - 1.)*k_inv*k_inv;
  }
  else
  {
  ...
  if(v > 0.1)
  {
    ...
    float dDdA = 0.5*D_inv*( 2.*(dx-x0)*ddxdA + 2.*(dy-y0)*ddydA );
...
void HelixKalman::calculate_dApdp(HelixKalmanState& state, Matrix<float,3,3>& dApdp, Vector3f& p, float phi, float cosphi, float sinphi)
{
  float k = state.kappa;
  float dzdl = state.dzdl;
  ...
  float pTneg3_2 = sqrt(pT_inv*pT_inv2);
  dApdp(1,0) = -0.003*Bfield*px*pTneg3_2;
  // dk/dpy
  dApdp(1,1) = -0.003*Bfield*py*pTneg3_2;
...
// dp/db : b is defined by p = R*s*b , with R being a rotation matrix rotating the unit vector in the z-direction to the direction of p, and S is a scalar with a value of the magnitude of the current value of p
...
// db/dt , with b a vector, and t a being the two rotation angles, one around the x axis and one around the y axis.  The derivative db/dt is calculated at the value of b being the unit vector in the z direction
void HelixKalman::calculate_dbdt(Matrix<float,3,2>& dbdt_out)
{
  ...
  // dbz/dt1 = -[(1 + [tan(t1)]^2 + [tan(t2)]^2)^(-3/2)]*( tan(t1)*( 1 + [tan(t1)]^2 ) )
...
void HelixKalman::calculateMSCovariance(HelixKalmanState& state, Matrix<float,5,5>& Q)
{
  ...
  float var=0.;
  if(calculateScatteringVariance(state, var) == false){return;}
